...
router.get("/queryCarting", function (req, res) {
    var page = new Page({
        page: req.query.page ? parseInt(req.query.page) : 0,
        size: req.query.pageSize ? parseInt(req.query.pageSize) : 5,
    })
    Product.queryCarting(page, function (err, data) {
...
router.post("/deleteProduct", function (req, res) {
    console.log('删除请求：',req.body);
    Product.deleteProduct(req.body.id, function (err, data) {
        if (err) return res.send({ "error": 403, "message": "数据库异常！" });
...
//更改当前物品上下架
// router.post("/updateStatu", checkRootLogin);
router.post("/updateStatu", function (req, res) {
    console.log('下架请求：',req.body);
    var product = new Product({
        id: req.body.id,
        statu: req.body.statu
    });
    Product.updateStatu(product, function (err, result) {
        if (err) return res.send({ "error": 403, "message": "数据库异常!" });
        res.send({ "success": true });
    });
});
