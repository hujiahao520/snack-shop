...
router.get("/userlogis", function (req, res) {  
        var  uid=req.session.user.id
        Address.findUserLogisticsModel(uid, function (err, data) {
            console.log("进来了吗？",uid)
 ...
 router.get("/search", function (req, res) {  
    Address.findAllLogisticsModel(req.query.id, function (err, result) {
        if (err) return res.send({ "error": 403, "message": "数据库异常！" });
        res.json({
            data : result
        });
...
router.get("/userlogisPhoto", function (req, res) {  
    Address.findLogisticsPhotoModel(req.query.id, function (err, result) {
        if (err) return res.send({ "error": 403, "message": "数据库异常！" });
        res.json({
            data : result
        });
...
router.post("/addAddress", function (req, res) {
    ...
    Address.addAddress(address, function (err, data) {
        if (err) return res.json({ "error": 403, "message": "数据库异常！" });
...
router.post("/deleteAddress", function (req, res) {
    Address.deleteAddress(req.body.id, function (err, data) {
        console.log('你们好啊',req.body.id);
        if (err) return res.send({ "error": 403, "message": "数据库异常！" });
        res.send({ "success": true });
    })
});

// router.post("/change",checkUserLogin);
...
router.post("/setDefault",checkUserLogin);
router.post("/setDefault", function (req, res) {
    Address.setDefault(req.body.id, function (err, data) {
        console.log(req.body.id);
        if (err) return res.send({ "error": 403, "message": "数据库异常！" });
        res.send({ "success": true });
    })
