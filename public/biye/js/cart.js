$(function () {
    ...
    getAddressData(function(e){
        window.e = e;
        $('.cart02-address').html(template('add',{model:e}));
        console.log(e);
    });
...
var setAmount = function () {
    ...
    if(Math.floor(amountSum * 100)%100===0){
...
$('body').on('click','.zhifu',buy)
function buy() {// 立即下单
    var shopArr = [];
    

    var amount = $('.cartAmount').html()
  
    var num = $(".mui-table-view").children("li").length; // 获取渲染出tr的数量，即商品的种类   
    for (var i = 0; i < num; i++) {
      if ($(".mui-table-view").children("li").eq(i).children().eq(1).children().eq(0).children().eq(0).children().eq(0).children("input").is(":checked")) {// 只传递被选中的数据
        var id = $(".mui-table-view").children("li").eq(i).children().eq(1).children().eq(0).children().eq(0).children().eq(0).children("input").attr("data-id")// 购物车里的购物id
        shopArr.push(id)        
        window.location.href = 'accountSettled.html?shopCart_id=' + shopArr +'?allPrice=' + amount;
      }
    ...
    // console.log()
    if (Number($(".shopcount").text()) == 0) { // 未勾选任何商品时
      layui.use('layer', function () {
        var layer = layui.layer;
  
        layer.msg("请勾选所要购买的商品");
      });
