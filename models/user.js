...
User.alterData = function(options,callback){
	var selectSql = `UPDATE user SET u_name = ?,u_birth = ?,u_sex = ?,u_img = ? WHERE id=? `;
	db.query(selectSql, options, function (err, data) {
		if (err) {
			return callback(err);
		}
		callback(err, data);
...
User.alterTel = function(id,username,mobile,callback){
	
	var selectSql = `UPDATE user SET username = ?, mobile = ? WHERE id=? `
	db.query(selectSql, [id,username,mobile], function(err,result){
		if (err) {
			return callback(err);
		}
		callback(err, result);
...
User.getPwd= function (id, callback) {
